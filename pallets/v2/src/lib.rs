...
#[derive(PartialEq, Eq, Clone, Encode, Decode, sp_runtime::RuntimeDebug)]
pub struct Collateral<AccountId, Balance> {
    pub account: AccountId,
    pub bonded: Balance,
}

#[derive(PartialEq, Eq, Encode, Decode, sp_runtime::RuntimeDebug)]
pub struct Node<TreeId, AccountId, Balance, State> {
    pub id: TreeId,
    // this param allows for easily decrementing parent's kid count when this tree is removed
    pub parent: Option<TreeId>,
    // ONLY account that can close tree (and all subtrees)
    pub controller: AccountId,
    pub height: u32,
    pub kids: OrderedSet<TreeId>,
    pub members: OrderedSet<Collateral<AccountId, Balance>>,
    pub state: State,
}
...
pub trait Trait: System {
    ...
    /// Bond amount for adding new members, m in y = mx + b
    type LinearBond: Get<BalanceOf<Self>>;

    /// Maximum number of members that can be associated with each Node (and delegate subtrees)
    type MaxMems: Get<u32>;

    /// Maximum depth for the lowest subtree
    type MaxDepth: Get<u32>;

    /// Maximum number of subtrees for every tree
    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn create_root(
            ...
            let state = TreeState {
                ...
                height: 0u32,
                members: OrderedSet::from(vec![Collateral { account: caller.clone(), bond }]),
                kids: OrderedSet::new(),
...
impl<T: Trait> Module<T> {
    ...
    pub fn reserve_exponential_bond(
        ...
        let bond: BalanceOf<T> = T::Bond::get().saturating_pow(exp);
        T::Currency::reserve(account, bond)?;
        tree.members.0
            .for_each(|a| {
                if &a.account == account {
                    *a.bond.saturating_add(bond);
                }
            });
